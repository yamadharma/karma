...
SRC_URI="${SRC_URI}
	...
	kickoff? (
		http://distfiles.gentoo-xeffects.org/kickoff/$KMNAME-3.5.7-$PN-kickoff-suse-$PATCH_VER.diff.bz2
		http://distfiles.gentoo-xeffects.org/kickoff/kickoff-icons.tar.bz2
	)"
...
src_unpack() {
	...
	if use kickoff; then
		# Add Kickoff support
		epatch "${DISTDIR}/$KMNAME-3.5.7-$PN-kickoff-suse-$PATCH_VER.diff.bz2"

		# Adjustments for Gentoo
		epatch "${FILESDIR}/kickoff-gentoo-xeffects-integration-$PATCH_VER.patch"

		# copy icons over
		tar xjf ${DISTDIR}/kickoff-icons.tar.bz2 -C ${WORKDIR}/${PN}-${PV}/${PN}/data
	fi
}
