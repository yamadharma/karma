...
SRC_URI="https://static.mendeley.com/bin/desktop/mendeley-reference-manager-${PV}-x86_64.AppImage"
...
src_unpack() {
	cp "${DISTDIR}"/mendeley-reference-manager-${PV}-x86_64.AppImage . || die "cp failed"
	chmod +x mendeley-reference-manager-${PV}-x86_64.AppImage || die "chmod failed"
	./mendeley-reference-manager-${PV}-x86_64.AppImage --appimage-extract || die "AppImage extract failed"
}
