...
src_unpack() {
	...
	mkdir "${WORKDIR}"/ooo/infra
	mv infra-ooo-files_${PV}/* "${WORKDIR}"/ooo/infra/

	# FIXME! Dirty hack
	mkdir -p "${WORKDIR}"/ooo/build/${MST}/sax/source
	cd "${WORKDIR}"/ooo/build/${MST}/sax/source
	ln -s ../../unoxml/source/fastsax fastparser
