...
# $Header: /var/cvsroot/gentoo-x86/media-gfx/blender/blender-2.59.ebuild,v 1.3 2011/09/20 02:49:30 Oicraman Exp $
...
src_prepare() {
	...
	# TODO: write a proper Linux-SConstruct patch to solve compilation with current linux kernel version 3.0 
	epatch "${FILESDIR}"/${PN}-${SLOT}-sconstruct.patch

	mv "${WORKDIR}"/${P}/build_files/scons/config/linux2-config.py "${WORKDIR}"/${P}/build_files/scons/config/linux-config.py
}
