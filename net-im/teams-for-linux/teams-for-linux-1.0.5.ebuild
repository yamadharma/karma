...
#	fperms +x /usr/share/teams/teams
	...
	# use system ffmpeg
	rm -f "${D}"/opt/${PN}/libffmpeg.so || die
...
#	doenvd 99teams
	...
	# use system mesa
	rm -f "${D}"/opt/${PN}/libEGL.so || die
	rm -f "${D}"/opt/${PN}/libGLESv2.so || die
	rm -f "${D}"/opt/${PN}/swiftshader/libEGL.so || die
	rm -f "${D}"/opt/${PN}/swiftshader/libGLESv2.so || die
