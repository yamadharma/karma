...
SRC_URI="http://www.process-one.net/en/projects/ejabberd/download/${PV}/${P}.tar.gz
	http://realloc.spb.ru/files/ejabberd/ejabber_ad_001.patch.gz"
LICENSE="GPL-2"
SLOT="0"
KEYWORDS="x86 amd64 ~sparc"
IUSE="mod_pubsub mod_irc mod_muc ldap web ssl odbc"
...
src_unpack ()
{
	unpack ${A}
	cd ${S}
	epatch ${WORKDIR}/ejabber_ad_001.patch
	
	sed -ie "s:/var/lib/ejabberd:/usr/lib/ejabberd:g" Makefile.in
}
...
src_compile() {
	...
	myconf="${myconf} `use_enable odbc`"
	
	myconf="${myconf} --enable-roster-gateway-workaround"
...
src_install() {
	...
	exeinto /etc/init.d
	newexe ${FILESDIR}/ejabberd.initd ejabberd
	if use ssl ; then
	...
	insinto /etc/conf.d
	newins ${FILESDIR}/ejabberd.confd ejabberd
