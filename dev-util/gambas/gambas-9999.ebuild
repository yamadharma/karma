...
inherit autotools eutils fdo-mime qt3 multilib subversion toolchain-funcs
...
KEYWORDS=""
IUSE="
	bzip2 cairo corba curl debug doc examples firebird gnome gtk kde mysql odbc
	opengl pcre pdf postgres qt3 qt4 sdl smtp sqlite sqlite3 svg v4l xml zlib
"

COMMON_DEPEND="
	bzip2?	( >=app-arch/bzip2-1.0.5 )
	cairo?	( >=x11-libs/cairo-1.6.0 )
	...
	qt4?	(
		>=x11-libs/qt-core-4.3.4:4
		>=x11-libs/qt-gui-4.3.4:4
		>=x11-libs/qt-qt3support-4.3.4:4
		opengl?	( >=x11-libs/qt-opengl-4.3.4:4 )
	)
	sdl?	(
...
#	use gtk; } ||	my_reduce_eautoreconf desktop
	use qt4 ||	my_reduce_eautoreconf qt4
	use cairo ||	my_reduce_eautoreconf cairo
	use gtk ||	my_reduce_eautoreconf imageio

	use doc ||	my_reduce_eautoreconf help
	use examples ||	my_reduce_eautoreconf examples
	eend 0

	ebegin "Fixing gambas-database-manager name"
	sed -i -e "s/gambas-database-manager/${MY_PN}-database-manager/" \
	app/src/INSTALL \
	|| die "sed gambas-database-manager_name-patch failed"
	mv app/src/gambas-database-manager app/src/${MY_PN}-database-manager \
	|| die "mv gambas-database-manager failed"
	eend 0

	# Needed in the SVN ebuilds
