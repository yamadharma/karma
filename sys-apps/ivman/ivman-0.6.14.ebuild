...
src_prepare() {
	...
	epatch "${FILESDIR}"/checkOnInit.patch 
	epatch "${FILESDIR}"/drop-privs-before-checkOnInit.patch
	epatch "${FILESDIR}"/fix-mountpoint-loop.patch 
	epatch "${FILESDIR}"/fix-nonexisting-cdrom.patch 
	epatch "${FILESDIR}"/less-verbose.patch 
	epatch "${FILESDIR}"/pidfile.patch
}
...
pkg_postinst() {
	...
	mkdir /var/run/ivman
	chown ivman:plugdev /var/run/ivman
}
