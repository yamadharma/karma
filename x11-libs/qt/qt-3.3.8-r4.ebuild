...
SRC_URI="ftp://ftp.trolltech.com/qt/source/qt-x11-${SRCTYPE}-${PV}.tar.gz
	qt-copy? (
	http://distfiles.gentoo-xeffects.org/qt-copy/qt-x11-${SRCTYPE}-qt-copy-${QT_COPY_SNAP}-${PV}.patch.bz2 )
...
src_unpack() {
	...
	# KDE related patches
	if ! use qt-copy ; then
		# These can only be applied if we do not use qt-copy - qt-copy contains
		# these already, so we can't double apply the patches.
		...
		epatch "${FILESDIR}"/utf8-bug-qt3.diff # 0077-utf8-decoder-fixes.diff
	fi
