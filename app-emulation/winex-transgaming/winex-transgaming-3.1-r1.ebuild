...
# $Header: /home/cvsroot/gentoo-x86/app-emulation/winex-transgaming/winex-transgaming-3.1.ebuild,v 1.3 2003/09/09 07:12:55 sergey Exp $
...
src_unpack () 
{
    if [ ! -e "${DISTDIR}/${MY_P}.tgz" && ! -e "${DISTDIR}/${MY_P}.tar.bz2" ] ; 
	then
	eerror ""
    ...
    if [ ! -e "${DISTDIR}/${MY_P}.tgz" ]
	then
	unpack "${MY_P}.tgz"
    elif [ ! -e "${DISTDIR}/${MY_P}.tar.bz2" ]
	then
	unpack "${MY_P}.tar.bz2"
    fi	
}
