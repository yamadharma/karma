...
# $Header: /var/cvsroot/gentoo-x86/media-fonts/terminus-font/terminus-font-4.14.ebuild,v 1.1 2005/05/29 20:14:52 usata Exp $

inherit gnustep-wrapper
...
src_install() {
	...
	if ( use gnustep )
	    ...
	    cp ${S}/*.bdf ${D}/usr/share/fonts/bdf/public/terminus
    	    gnustep_wrapper_nfont_install ${FILESDIR}/nfont.tar.bz2
	fi
}
...
fonts_pkg_postinst () 
{
        if ( use X )
	    then	
    	    /usr/sbin/chkfontpath -q -a /usr/share/fonts/pcf/public/terminus
...
fonts_pkg_prerm () 
{
        ...
        if ( use X )
	    then	
	    /usr/sbin/chkfontpath -r /usr/share/fonts/pcf/public/terminus
	    /etc/init.d/xfs restart
        fi
}
