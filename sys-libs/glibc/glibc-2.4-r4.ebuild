...
IUSE="nls build nptl nptlonly hardened multilib selinux glibc-omitfp profile nomalloccheck debug hashstyle"
...
#	               http://ftp.gnu.org/gnu/glibc/glibc-libidn-${GLIBC_RELEASE_VER}.tar.bz2
	...
	if [[ -n ${EXTRA_PATCH_VER} ]] ; then
		GLIBC_SRC_URI="${GLIBC_SRC_URI}
			${GENTOO_TOOLCHAIN_BASE_URI}/glibc-${GLIBC_RELEASE_VER}-extra_patches-${EXTRA_PATCH_VER}.tar.bz2
                        ${GENTOO_TOOLCHAIN_DEV_URI//XXX/glibc-${GLIBC_RELEASE_VER}-extra_patches-${EXTRA_PATCH_VER}.tar.bz2}"
	fi
...
toolchain-glibc_src_unpack() {
	...
	if [[ -n ${EXTRA_PATCH_VER} ]] ; then
		cd "${WORKDIR}"
		unpack glibc-${GLIBC_RELEASE_VER}-extra_patches-${EXTRA_PATCH_VER}.tar.bz2
	fi
	...
	if [[ -n ${EXTRA_PATCH_VER} ]] ; then
		cd "${S}"
		EPATCH_MULTI_MSG="Applying Extra Glibc Patchset ${EXTRA_PATCH_GLIBC_VER:-${GLIBC_RELEASE_VER}}-${EXTRA_PATCH_VER} ..." \
		EPATCH_EXCLUDE=${EXTRA_PATCH_EXCLUDE} \
		EPATCH_SUFFIX="patch" \
		ARCH=$(tc-arch) \
...
DEPEND=">=sys-devel/gcc-3.4.4
	...
	hashstyle? ( >=sys-devel/binutils-2.17.50.0.3 )
	!hashstyle ( >=sys-devel/binutils-2.17 )
...
src_unpack() {
	...
	if useq hashstyle; then
		GLIBC_PATCH_EXCLUDE="${GLIBC_PATCH_EXCLUDE} 6910_all_glibc-suse-hashvals.patch"
		EXTRA_PATCH_EXCLUDE="${EXTRA_PATCH_EXCLUDE} 1041_all_glibc-suse-bdirect.patch"
	else
		EXTRA_PATCH_EXCLUDE="${EXTRA_PATCH_EXCLUDE} 1050_all_glibc-2.4-gnu-hash.patch 1051_all_glibc-2.4-gnu-hash-hack.patch \
		1052_all_glibc-2.4-gnu-hash-fix.patch 1053_all_glibc-2.4-gnu-hash-fix2.patch"
	fi

	toolchain-glibc_src_unpack

	[[ $(gcc-major-version) == "4" ]] && epatch ${FILESDIR}/2.4/glibc-2.4-rtld-stack-protector.patch
