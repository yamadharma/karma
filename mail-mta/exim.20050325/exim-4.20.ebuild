...
SRC_URI="ftp://ftp.exim.org/pub/exim/exim4/${P}.tar.gz 
  mirror://sourceforge/sa-exim/sa-exim-${SA_EXIM_V}.tar.gz
  http://duncanthrax.net/exiscan-acl/exiscan-acl-${EXISCAN_V}.patch.bz2"
...
src_unpack() 
{
    ...
    epatch ../sa-exim-${SA_EXIM_V}/localscan_dlopen.patch
#    epatch ${DISTDIR}/exiscan-acl-${EXISCAN_V}.patch
    bzcat ${DISTDIR}/exiscan-acl-${EXISCAN_V}.patch.bz2 | patch -p1
...
#	doins configure.default
  ...
  dodir /etc/exim
  cp -R ${FILESDIR}/etc/version/${PV}/* ${D}/etc

  cd ${S}
  dodoc ACKNOWLEDGMENTS CHANGES LICENCE NOTICE README*
  dodoc ${S}/doc/*
  doman ${S}/doc/exim.8
...
#	  newins spamassassin.conf sa-exim.conf.example
  ...
  docinto sa-exim
  dodoc  README INSTALL LICENCE TODO *.conf
  dohtml sa.html Acknowledgements.html Changelog.html 
	  
  docinto sa-exim/contrib
  dodoc contrib/*
...
#	fi
  ...
  dodir /var/spool/exim/sa
  chown mail:mail ${D}/var/spool/exim/sa
  #}}}
}
...
pkg_postinst() 
{
  ...
  if [ ! -e /etc/mailname ]; then 
    /bin/hostname --fqdn > /etc/mailname ; 
  fi 
  ##
  ...
  einfo "Please create /etc/exim/configure from /etc/exim/configure.example."
