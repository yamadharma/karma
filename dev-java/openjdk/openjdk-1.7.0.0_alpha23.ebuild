...
src_unpack() {
	...
	rm -r jdk/src/share/native/java/util/zip/zlib-1.1.3 || die
	echo "Deleting bundled libpng"
	rm -r jdk/src/share/native/sun/awt/libpng || die
	...
	# bundled
# Presently MIA, above grep does show others, but leaving them for now
#	cd jdk/src/solaris/native/sun/awt
#	rm -v randr.h panoramiXext.h panoramiXproto.h Xrandr.h extutil.h \
#		Xinerama.c Xinerama.h || die
	cd "${S}"

# Unsed/Obsolete patches
# execstack external-libXinerama-b17

	PATCHES="external-zlib-b23 external-zlib-splash-b23 external-jpeg-splash-b23 \
		external-giflib-b23 external-libpng-b23 gettimeofday-declaration-b23 jdk-cxxflags-b23 \
		hotspot-cflags cpuid-fomit-frame-pointer-b23 icedtea-text-relocations \
		tempname-free-b23 fontconfig-directlink-b23"
...
src_compile() {
	...
	if ! emake ${make} -j1 \
		...
		BUILD_NUMBER=gentoo-b${ALPHA} ; then
