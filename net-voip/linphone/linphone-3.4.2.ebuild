...
# $Header: /var/cvsroot/gentoo-x86/net-voip/linphone/linphone-3.3.2.ebuild,v 1.4 2011/01/01 20:36:52 hwoarang Exp $
...
RDEPEND=">=media-libs/mediastreamer-2.7.2[video?]
...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-3.4.1-automake.patch
	epatch "${FILESDIR}"/${PN}-3.4.1-nls.patch

	# remove speex check, avoid bug when mediastreamer[-speex]
	epatch "${FILESDIR}"/${PN}-3.4.1-speex.patch
