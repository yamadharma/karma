...
src_unpack ()
{
	...
	case ${CHOST} in
		*-darwin*) 
		    ;;
		*)         
		    epatch ${FILESDIR}/rm_darwin.patch
		    ;;
	esac
...
src_install () 
{
	...
	dolib.so lib/libportaudio$(get_libname 0.0.19)
	dolib.a lib/libportaudio.a 
	
	mv ${D}/usr/$(get_libdir)/libportaudio$(get_libname 0.0.19) ${D}/usr/$(get_libdir)/libportaudio-19$(get_libname 0.0.19)
	mv ${D}/usr/$(get_libdir)/libportaudio.a ${D}/usr/$(get_libdir)/libportaudio-19.a
	
	dosym /usr/$(get_libdir)/libportaudio-19$(get_libname 0.0.19) /usr/$(get_libdir)/libportaudio-19$(get_libname 0)
	dosym /usr/$(get_libdir)/libportaudio-19$(get_libname 0.0.19) /usr/$(get_libdir)/libportaudio-19$(get_libname)
