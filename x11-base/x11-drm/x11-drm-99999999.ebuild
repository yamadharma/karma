...
src_unpack() {
	...
	cd "${EGIT_STORE_DIR}"
	GIT_REVISION="`git-describe --abbrev=17`"
...
pkg_postinst() {
	...
	then
		elog "SiS direct rendering only works on 300 series chipsets."
		elog "SiS framebuffer also needs to be enabled in the kernel."
	fi

	if use video_cards_mach64
	then
		elog "The Mach64 DRI driver is insecure."
		elog "Malicious clients can write to system memory."
		elog "For more information, see:"
		elog "http://dri.freedesktop.org/wiki/ATIMach64."
	fi
...
kernel_setup() {
	...
	then
		if use video_cards_nouveau
		then
			eerror "Nouveau does not support FreeBSD"
			die "Nouveau does not support FreeBSD"
		else
			K_RV=${CHOST/*-freebsd/}
		fi
