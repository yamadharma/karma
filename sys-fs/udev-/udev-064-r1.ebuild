...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/udev/udev-064-r1.ebuild,v 1.1 2005/07/31 07:01:29 gregkh Exp $
...
src_unpack() {
	...
	if [ "`md5sum < "${S}/etc/udev/gentoo/udev.rules"`" != \
	     "e1f3c174a5592b0ffa02d1bc62864326  -" ]
	then
		echo
...
src_compile() {
	local myconf=
	local extras="extras/scsi_id extras/volume_id extras/ata_id extras/run_directory extras/usb_id extras/floppy"
...
src_install() {
	...
	doexe extras/usb_id/usb_id
	doexe extras/floppy/create_floppy_devices
	...
	# Our default /dev layout (no more devfs crap.)
	insinto /etc/udev/rules.d/
#	newins etc/udev/gentoo/udev.rules 50-udev.rules
	# use our own rules for now, till upstream catches up.
	newins ${FILESDIR}/udev.rules-064-r1 50-udev.rules
