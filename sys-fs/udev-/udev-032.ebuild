...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/udev/udev-032.ebuild,v 1.1 2004/09/14 06:20:53 gregkh Exp $
...
src_unpack() {
	...
	if ( use selinux )	
	    then
	    epatch ${FILESDIR}/udev-030-cloexec.patch
	    epatch ${FILESDIR}/udev-032-selinux.patch
	fi	    
}
...
pkg_postinst() {
	...
	if [ -f /etc/dev.d/default/selinux.dev ]
	    then
	    rm -f /etc/dev.d/default/selinux.dev
	fi

	if [ -f /etc/dev.d/snd/controlC0/alsa.dev ]
	    then
	    rm -f /etc/dev.d/snd/controlC0/alsa.dev
	fi
}
