...
fonts_pkg_postinst () 
{
    ...
    for format in ${FONT_FORMAT}
	  ...
	  ttf) 
	      if ( use X )
		  then	
		  /usr/sbin/chkfontpath -q ${CHKFONTPATH_OPTIONS} -a ${TTF_FONTSDIR}
	  ...
	  type1)
	      if ( use X )
		  then	
		  /usr/sbin/chkfontpath -q ${CHKFONTPATH_OPTIONS} -a ${TYPE1_FONTSDIR}
		  /etc/init.d/xfs restart
	      fi
	      ;;    	      
	  psf) 
	  ...
	  pcf|bdf) 
	      if ( use X )
		  then	
		  /usr/sbin/chkfontpath -q ${CHKFONTPATH_OPTIONS} -a ${PCF_FONTSDIR}
		  /etc/init.d/xfs restart
	      fi
	      ;;
	  *) 
...
fonts_pkg_prerm () 
{
    ...
    for format in ${FONT_FORMAT}
	  ...
	  ttf) 
	      ...
	      if ( use X )
		  then	
		  /usr/sbin/chkfontpath -r ${TTF_FONTSDIR}
		  /etc/init.d/xfs restart
	      fi
	      ;;
	  type1) 
	  ...
	  pcf|bdf|bdf2pcf) 
	      ...
	      if ( use X )
		  then	
		  /usr/sbin/chkfontpath -r ${PCF_FONTSDIR}
		  /etc/init.d/xfs restart
	      fi
	      ;;
	  *) 
