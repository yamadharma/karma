...
# $Header: /var/cvsroot/gentoo-x86/eclass/kde4overlay-base.eclass,v 1.6 2008/03/26 20:39:05 zlin Exp $

# @ECLASS: kde4overlay-base.eclass
# @MAINTAINER:
# kde@gentoo.org
# @BLURB: This eclass provides functions for kde 4.0 ebuilds
# @DESCRIPTION:
# The kde4overlay-base.eclass provides support for building KDE4 monolithic ebuilds
...
inherit base eutils multilib cmake-utils kde4overlay-functions

EXPORT_FUNCTIONS pkg_setup src_unpack src_compile src_test src_install pkg_postinst pkg_postrm

kde4overlay-base_set_qt_dependencies() {
	local qt qtcore qtgui qt3support qtscript qtsvg qttest qtopengl qtdepend qtopengldepend
...
if [[ -z ${NEED_KDE} ]]; then
	if [[ -n ${KDEBASE} ]]; then
		...
		die "kde4overlay-base.eclass inherited but NEED_KDE not defined - broken ebuild"
	fi
fi
export NEED_KDE
...
if [[ ${NEED_KDE} != none ]]; then
	...
	if [[ -n ${KDEBASE} ]]; then
		# removing slot kde-4 as we don't use it and koffice split builds barf otherwise and I'm lazy
...
# can be superseded by defining PREFIX before inheriting kde4overlay-base.
if [[ -n ${KDEBASE} ]]; then
	PREFIX=${KDEDIR}
else
...
# @FUNCTION: kde4overlay-base_pkg_setup
# @DESCRIPTION:
# Adds flags needed by all of KDE 4 to $QT4_BUILT_WITH_USE_CHECK. Uses
# kde4overlay-functions_check_use from kde4overlay-functions.eclass to print appropriate
...
kde4overlay-base_pkg_setup() {
	debug-print-function $FUNCNAME "$@"
		...
		if [[ $(declare -p KDE4_BUILT_WITH_USE_CHECK 2>/dev/null) != 'declare -a '* ]]; then
			while read line; do
				[[ -z ${line} ]] && continue
		...
		kde4overlay-functions_check_use
}

# @FUNCTION: kde4overlay-base_apply_patches
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-base_src_unpack
# @DESCRIPTION:
...
#
# In addition it calls kde4overlay-base_apply_patches when no arguments are passed to
# this function.
#
# It also handles translations if KDE_LINGUAS is defined. See KDE_LINGUAS and
# enable_selected_linguas() in kde4overlay-functions.eclass(5) for further details.
...
# @FUNCTION: kde4overlay-base_src_compile
# @DESCRIPTION:
# General function for compiling KDE4 applications.
kde4overlay-base_src_compile() {
	debug-print-function ${FUNCNAME} "$@"

	kde4overlay-base_src_configure
	kde4overlay-base_src_make
}

# @FUNCTION: kde4overlay-base_src_configure
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-base_src_make
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-base_src_test
# @DESCRIPTION:
# Function for testing KDE4 applications.
kde4overlay-base_src_test() {
	debug-print-function ${FUNCNAME} "$@"

	# Override this value, set in kde4overlay-base_src_configure()
...
# @FUNCTION: kde4overlay-base_src_make_doc
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-base_pkg_postinst
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-base_pkg_postrm
# @DESCRIPTION:
# Function to rebuild the KDE System Configuration Cache after an application has been removed.
kde4overlay-base_pkg_postrm() {
	buildsycoca
}
