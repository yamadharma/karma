...
# $Header: /var/cvsroot/gentoo-x86/eclass/kde4overlay-meta.eclass,v 1.5 2008/03/13 17:57:51 ingmar Exp $
#
# @ECLASS: kde4overlay-meta.eclass
# @MAINTAINER:
# kde@gentoo.org
...
inherit multilib kde4overlay-functions kde4overlay-base

EXPORT_FUNCTIONS pkg_setup src_unpack src_compile src_test src_install pkg_postinst pkg_postrm

if [[ -z ${KMNAME} ]]; then
	die "kde4overlay-meta.eclass inherited but KMNAME not defined - broken ebuild"
fi
...
case ${KMNAME} in
	...
	;;
	kdenetwork)
		DEPEND="${DEPEND} kde-base/kdepimlibs:${SLOT}"
	;;
	kdeutils)
		DEPEND="${DEPEND} kde-base/kdepimlibs:${SLOT}"
	;;
...
# @DESCRIPTION:
# Specify extra parameters to pass to tar, in kde4overlay-meta_src_extract.
# '-xpf -j' are passed to tar by default.

# @FUNCTION: kde4overlay-meta_pkg_setup
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-meta_src_unpack
# @DESCRIPTION:
# This function unpacks the source for split ebuilds. See also
# kde4overlay-meta-src_extract.
kde4overlay-meta_src_unpack() {
	debug-print-function  ${FUNCNAME} "$@"

	kde4overlay-meta_src_extract
	kde4overlay-meta_change_cmakelists
}

# @FUNCTION: kde4overlay-meta_src_extract
# @DESCRIPTION:
...
kde4overlay-meta_create_extractlists() {
	...
	case ${KMNAME} in
		kdebase-workspace|kdebase-runtime|kdepim|kdegames|kdegraphics|kdeedu)
			case ${PN} in
...
# @FUNCTION: kde4overlay-meta_src_compile
# @DESCRIPTION:
# General function for compiling split KDE4 applications.
kde4overlay-meta_src_compile() {
	debug-print-function  ${FUNCNAME} "$@"

	kde4overlay-meta_src_configure
...
# @FUNCTION: kde4overlay-meta_src_configure
# @DESCRIPTION:
# Currently just calls its equivalent in kde4-base.eclass(5). Use this in split
# ebuilds.
kde4overlay-meta_src_configure() {
...
# @FUNCTION: kde4overlay-meta_src_make
# @DESCRIPTION:
# Currently just calls its equivalent in kde4-base.eclass(5). Use this in split
# ebuilds.
kde4overlay-meta_src_make() {
...
# @FUNCTION: kde4overlay-meta_src_test
# @DESCRIPTION:
# Currently just calls its equivalent in kde4-base.eclass(5). Use this in split
# ebuilds.
...
# @FUNCTION: kde4overlay-meta_src_make_doc
# @DESCRIPTION:
...
# @FUNCTION: kde4overlay-meta_pkg_postinst
# @DESCRIPTION:
# Currently just calls its equivalent in kde4-base.eclass(5). Use this in split
# ebuilds.
...
# @FUNCTION: kde4overlay-meta_pkg_postrm
# @DESCRIPTION:
# Currently just calls its equivalent in kde4-base.eclass(5). Use this in split
# ebuilds.
kde4overlay-meta_pkg_postrm() {
