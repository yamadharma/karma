...
# -----------------------------------------------------------------------------
JAVA_PKG_NATIVE_BIN_FLAGS="-Wl,-rpath=/usr/lib/gcj/lib -Wl,-Bsymbolic -findirect-dispatch -fjni"
...
# ------------------------------------------------------------------------------
java-pkg_get-source() {
	echo ${JAVA_PKG_WANT_SOURCE:-$(depend-java-query --get-lowest "${DEPEND} ${RDEPEND}" 2>/dev/null)}
}
...
# ------------------------------------------------------------------------------
java-pkg_get-target() {
	echo ${JAVA_PKG_WANT_TARGET:-$(depend-java-query --get-lowest "${DEPEND} ${RDEPEND}" 2>/dev/null)}
}
...
java-pkg_native_init_() {
	...
	if [[ ! -d /usr/share/gcj-config ]]; then
		ewarn "gcj setup missing! run gcj-config ..."
		return 1
	fi
	if [[ ( ! -x "$(gcj-config --gcj)" ) || ( ! -x "$(gcj-config --dbtool)" ) ]] ; then
...
java-pkg_donative() {
	...
	for file in $(find -type f -name '*.java' | cut -c3-) ; do
		echo $(gcj-config --gcj) -c -g0 ${CFLAGS} -Wno-deprecated \
			${JAVA_PKG_NATIVE_INC} ${file} -o ${file/\.java/.o}
		$(gcj-config --gcj) -c -g0 ${CFLAGS} -Wno-deprecated \
			${JAVA_PKG_NATIVE_INC} ${file} -o ${file/\.java/.o} \
...
java-pkg_donative-bin() {
	...
	if [ ".jar" == "${2: -4:4}" ] ; then
		...
		echo $(gcj-config --gcj) ${JAVA_PKG_NATIVE_BIN_FLAGS} \
			--main=${1} -o ../${bin} ${LDFLAGS} \
			${JAVA_PKG_NATIVE_LIB} ...
		$(gcj-config --gcj) ${JAVA_PKG_NATIVE_BIN_FLAGS} \
			--main=${1} -o ../${bin} ${LDFLAGS} \
...
java-pkg_cachejar_() {
	...
	for jar in ${jars} ; do
		...
		if [[ ( -f "${D}${jar}" ) && ( ".jar" == "${jar: -4:4}" ) && ( ! -e "${D}${to}" ) ]] ; then
			echo $(gcj-config --gcj) ${JAVA_PKG_NATIVE_CACHE_FLAGS} \
				-g0 ${CFLAGS} -o ${to} ${jar}
			if ! $(gcj-config --gcj) ${JAVA_PKG_NATIVE_CACHE_FLAGS} \
...
java-pkg_reg-cachejar_() {
	...
	if [ ! -e "${JAVA_PKG_NATIVE_CLASSMAP}" ] ; then
		...
		$(gcj-config --dbtool) -n ${JAVA_PKG_NATIVE_CLASSMAP}
	fi
	...
	for jar in ${JAVA_PKG_CLASSPATH//:/ } ; do
		...
		if [[ ( -f "${jar}" ) && ( ".jar" == "${jar: -4:4}" ) && ( -f "${to}" ) ]] ; then
			einfo "library: ${to}"
			$(gcj-config --dbtool) -a ${JAVA_PKG_NATIVE_CLASSMAP} ${jar} ${to} \
