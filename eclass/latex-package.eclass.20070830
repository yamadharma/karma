...
S=${WORKDIR}/${P}
TEXMF=`kpsewhich -expand-var='$TEXMFSITE'`
SUPPLIER="misc" # This refers to the font supplier; it should be overridden
BUNDLE=${BUNDLE:-$PN}

latex-package_has_tetex_3() {
	if has_version '>=app-text/tetex-3' || has_version '>=app-text/ptex-3.1.8' || has_version '>=app-text/texlive-2005' ; then
		true
	else
		false
	fi
...
{
    if latex-package_has_tetex_3
	then
	TEXMF=`kpsewhich -expand-var='$TEXMFSITE'`
	export TEXMF=${TEXMF:-`kpsewhich -expand-var='$TEXMFDIST'`}
	export VARTEXMF=`kpsewhich -expand-var='$TEXMFSYSVAR'`
        export VARTEXFONTS=`kpsewhich -expand-var='$VARTEXFONTS'`
    else
	export TEXMF=`kpsewhich -expand-var='$TEXMFMAIN'`
	export VARTEXMF=`kpsewhich -expand-var='$VARTEXMF'`
        export VARTEXFONTS=`kpsewhich -expand-var='$VARTEXFONTS'`
    fi
}
	...
	while [ "$1" ]; do
		case $1 in
			...
			"sty" | "cls" | "fd" | "clo" | "def" | "cfg")
				for i in `find . -maxdepth 1 -type f -name "*.${1}"`
				do
					insinto ${TEXMF}/tex/latex/${BUNDLE}
			...
			"tfm" | "vf" | "afm")
				for i in `find . -maxdepth 1 -type f -name "*.${1}"`
				do
					insinto ${TEXMF}/fonts/${1}/${SUPPLIER}/${BUNDLE}
					doins $i || die "doins $i failed"
				done
				;;
			"pfb")
				...
				do
					insinto ${TEXMF}/fonts/type1/${SUPPLIER}/${BUNDLE}
					doins $i || die "doins $i failed"
				done
				;;
			"ttf")
				...
				do
					insinto ${TEXMF}/fonts/truetype/${SUPPLIER}/${BUNDLE}
					doins $i || die "doins $i failed"
				done
				;;
			"bst")
				...
				do
					insinto ${TEXMF}/bibtex/bst/${BUNDLE}
					doins $i || die "doins $i failed"
				done
				;;
			"styles")
