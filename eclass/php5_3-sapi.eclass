...
if [[ "${PHP_PACKAGE}" == 1 ]] ; then
	...
	LICENSE="PHP-3"
	SRC_URI="http://www.php.net/distributions/${PN}-${PV}.tar.bz2"
	S="${WORKDIR}/${PN}-${PV}"
fi
...
SRC_URI="${SRC_URI} http://perso.musedev.org/gentoo/php-patchset-${PV}-r${PHP_PATCHSET_REV}.tar.bz2"
...
php5_3-sapi_install_ini() {
	...
	elif [[ "${PHPSAPI}" == "fpm" ]] ; then
		phpmycnfcharset="`php_get_mycnf_charset fpm`"
		einfo "MySQL extensions charset for 'fpm' SAPI is: ${phpmycnfcharset}"
...
php5_3-sapi_src_unpack() {
	...
	if [[ -d "${WORKDIR}/${PV}/php5" ]] ; then
		EPATCH_SOURCE="${WORKDIR}/${PV}/php5" EPATCH_SUFFIX="patch" EPATCH_FORCE="yes" epatch
	fi

	# Apply version-specific PHP patches
	if [[ -d "${WORKDIR}/${PV}/${PV}" ]] ; then
		EPATCH_SOURCE="${WORKDIR}/${PV}/${PV}" EPATCH_SUFFIX="patch" EPATCH_FORCE="yes" epatch
	fi
