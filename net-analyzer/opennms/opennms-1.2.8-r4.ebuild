...
# Maintainer: Alexander Hoogerhuis <alexh@boxed.no>
#
# This ebuild come from http://portage.boxed.no/net-analyzer/opennms/ - The site http://gentoo.zugaina.org/ only host a copy.
...
SRC_URI="http://friends.grandiosa.net/~alexh/opennms-source-${PV}-1.tar.gz"
...
# FIXME: the deps makes tomcat inclue blackdown before opennms pulls in sun-jdk
RDEPEND=">=dev-java/sun-jdk-1.4.2.05
...
pkg_preinst() {
	...
	echo "CONFIG_PROTECT=\"${CONFIG_PROTECT} ${OPENNMS_DIR}/etc\"" >> ${D}/etc/env.d/21${PN}
}
...
pkg_config() {
	...
	if has_version '=www-servers/tomcat-5.0*' && [ -d /opt/tomcat5/webapps -a -d /opt/tomcat5/server/lib ]; then
		einfo ""
		einfo "* Installing the tomcat webapp:"
		${OPENNMS_DIR}/bin/install -y -w /opt/tomcat5/webapps -W /opt/tomcat5/server/lib
	fi
}
