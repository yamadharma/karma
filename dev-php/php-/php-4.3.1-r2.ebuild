...
# $Header: /home/cvsroot/gentoo-x86/dev-php/php/php-4.3.1-r1.ebuild,v 1.3 2003/02/24 21:55:22 rphillips Exp $
...
src_unpack() 
{
	...
	#	fi
	# fi
  #{{{ Caudium module install path fix

  cd ${S}
  patch	-p1 < ${FILESDIR}/patch/${PV}/p1--*
...
#	use truetype && myconf="${myconf} --with-ttf --with-t1lib"
	use truetype && myconf="${myconf} --with-freetype=/usr	--with-t1lib"
  ...
  if [ -d /var/db/pkg/net-www/caudium* ]
  then 
    myconf="${myconf} --with-caudium=/usr/lib/caudium"
  fi
  
##--with-caudium=/opt/caudium/server --enable-track-vars
...
src_install() 
{
  ...
  make INSTALL_ROOT=${D} install-cli install-pear install-headers install-programs install-modules install-build install-sapi || die
